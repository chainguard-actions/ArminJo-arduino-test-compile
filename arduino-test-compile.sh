...
if [[ -f $HOME/arduino_ide/arduino-cli ]]; then
  ...
  echo -n "downloading: "
  # Validate CLI_VERSION to prevent shell injection via URL construction
  if [[ ! "$CLI_VERSION" =~ ^[a-zA-Z0-9._-]+$ ]]; then
    echo -e "${RED}\xe2\x9c\x96"
    echo "::error:: Invalid CLI_VERSION value: ${CLI_VERSION}"
    exit 3
  fi
...
for single_platform in "${PLATFORM_ARRAY[@]}"; do # Loop over all platforms specified
  if [[ $DEBUG_INSTALL == true ]]; then
    echo -e "arduino-cli core update-index ${PLATFORM_URL_COMMAND:+$PLATFORM_URL_COMMAND} ${PLATFORM_URL:+\"$PLATFORM_URL\"} --verbose"
    arduino-cli core update-index ${PLATFORM_URL_COMMAND:+"$PLATFORM_URL_COMMAND"} ${PLATFORM_URL:+"$PLATFORM_URL"} --verbose # must specify --additional-urls here
    if [[ $? -ne 0 ]]; then
      echo "::error::Updating index of $PLATFORM_URL failed"
      exit 1
    fi
    echo -e "arduino-cli core install $single_platform ${PLATFORM_URL_COMMAND:+$PLATFORM_URL_COMMAND} ${PLATFORM_URL:+\"$PLATFORM_URL\"} -v"
    arduino-cli core install "$single_platform" ${PLATFORM_URL_COMMAND:+"$PLATFORM_URL_COMMAND"} ${PLATFORM_URL:+"$PLATFORM_URL"} --verbose
  else
    echo -e "arduino-cli core update-index ${PLATFORM_URL_COMMAND:+$PLATFORM_URL_COMMAND} ${PLATFORM_URL:+\"$PLATFORM_URL\"} > /dev/null"
    arduino-cli core update-index ${PLATFORM_URL_COMMAND:+"$PLATFORM_URL_COMMAND"} ${PLATFORM_URL:+"$PLATFORM_URL"} > /dev/null # must specify --additional-urls here
    if [[ $? -ne 0 ]]; then
      echo "::error::Updating index of $PLATFORM_URL failed"
      exit 1
    fi
    echo -e "arduino-cli core install $single_platform ${PLATFORM_URL_COMMAND:+$PLATFORM_URL_COMMAND} ${PLATFORM_URL:+\"$PLATFORM_URL\"} > /dev/null"
    arduino-cli core install "$single_platform" ${PLATFORM_URL_COMMAND:+"$PLATFORM_URL_COMMAND"} ${PLATFORM_URL:+"$PLATFORM_URL"} > /dev/null
  fi
    if [[ $? -ne 0 ]]; then
...
if [[ -z $REQUIRED_LIBRARIES ]]; then
  ...
  IFS="$BACKUP_IFS"
  # Convert EXTRA_ARDUINO_LIB_INSTALL_ARGS to an array for safe expansion
  read -ra EXTRA_LIB_ARGS_ARRAY <<< "$EXTRA_ARDUINO_LIB_INSTALL_ARGS"
  if [[ $DEBUG_INSTALL == true ]]; then
    echo "arduino-cli lib install \"${REQUIRED_LIBRARIES_ARRAY[@]}\" ${EXTRA_LIB_ARGS_ARRAY[*]}"
...
for sketch_name in "${SKETCH_NAMES_ARRAY[@]}"; do # Loop over all sketch names
  ...
  for sketch in "${SKETCHES[@]}"; do # Loop over all sketch files
    ...
    if [[ $SKETCHES_EXCLUDE == *"$SKETCH_BASENAME"* ]]; then
      ...
      # Build arrays for safe argument expansion
      # Convert EXTRA_ARDUINO_CLI_ARGS to an array for safe expansion
      read -ra EXTRA_CLI_ARGS_ARRAY <<< "$EXTRA_ARDUINO_CLI_ARGS"
      # Build BUILD_PATH_PARAMETER as an array to avoid word-splitting issues
      declare -a BUILD_PATH_PARAM_ARRAY=()
      if [[ $SET_BUILD_PATH == true ]]; then
        BUILD_PATH_PARAMETER="--build-path $SKETCH_PATH/build/"
        BUILD_PATH_PARAM_ARRAY=(--build-path "$SKETCH_PATH/build/")
      fi
      ...
      if [[ -z $GCC_EXTRA_FLAGS ]]; then
        build_stdout=$(arduino-cli compile --verbose --warnings all --fqbn "${ARDUINO_BOARD_FQBN%|*}" "${BUILD_PATH_PARAM_ARRAY[@]}" "${EXTRA_CLI_ARGS_ARRAY[@]}" "$SKETCH_PATH" 2>&1)
      else
        build_stdout=$(arduino-cli compile --verbose --warnings all --fqbn "${ARDUINO_BOARD_FQBN%|*}" "${BUILD_PATH_PARAM_ARRAY[@]}" --build-property "compiler.cpp.extra_flags=${GCC_EXTRA_FLAGS}" --build-property "compiler.c.extra_flags=${GCC_EXTRA_FLAGS}" --build-property "compiler.S.extra_flags=${GCC_EXTRA_FLAGS}" "${EXTRA_CLI_ARGS_ARRAY[@]}" "$SKETCH_PATH" 2>&1)
      fi
      if [[ $? -ne 0 ]]; then
